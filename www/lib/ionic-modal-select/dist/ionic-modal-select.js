    ...
    function modalSelect($ionicModal, $timeout, $filter, $parse, $templateCache) {
      ...
      return {
        ...
        link: function link(scope, iElement, iAttrs, ngModelController, transclude) {
          ...
          var optionsMultipleRender = iAttrs.optionsmultiplerender; //modificado Abraham, viene en minuscula todo no se porque

          if (optionsMultipleRender) { //modificado Abraham, viene en minuscula la r de render no se porque
            optionsMultipleRender = optionsMultipleRender.replace(new RegExp("{", 'g'), "{{").replace(new RegExp("}", 'g'), "}}"); //ñapa
            modalTemplateMultiple = modalTemplateMultiple.replace("<optionsMultipleRender>", optionsMultipleRender);
          }
          ...
          if (iAttrs.searchTemplate) {
            scope.modal = $ionicModal.fromTemplate($templateCache.get(iAttrs.searchTemplate), {
              scope: scope
            });
          } else {
            modalTpl = multiple ? modalTemplateMultiple : modalTemplate;
            scope.modal = $ionicModal.fromTemplate(modalTpl, {
              scope: scope
            });
          }
          ...
          if (scope.onClose && angular.isFunction(scope.onClose)) {
            hiddenCb = scope.$on('modal.hidden', function (event, modal) {
              scope.onClose();
            });
          }
          iElement.on('unCheckOptions', function () { //modificado Abraham, creo el evento unCheckOptions para deseleccionarlos
            scope.isChecked = {};
          });
          ...
          if (scope.ui.hasSearch) {
            scope.$watch('ui.searchValue', function (nv) {
              ...
              if (iAttrs.onSearch) {
                  ...
                  if (searchProperties) {
                    if ((typeof actual === 'undefined' ? 'undefined' : _typeof(actual)) == 'object') {
                      for (var i = 0; i < searchProperties.length; i++) {
                        ...
                        //modificado Abraham para permitir searchProperties que contenga enteros y quite los acentos en la busqueda
                        if (actual[searchProperties[i]] && actual[searchProperties[i]].toString().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "").indexOf(expected.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "")) >= 0) {
                          return true;
                        }
                      }
                    }
                    ...
                    if (actual.toString().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "").indexOf(expected.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "")) >= 0) {
                      return true;
                    }
                  }
                  return false;
                ...
                //if (!multiple) {
                scope.options = filteredOpts; //modificado Abraham para no hacer loops
                /*} else {
